...
export function RunsPage() {
  ...
  async function cancel(run: Run) {
    if (!window.confirm(`Stop ${run.name}? Pending jobs will be removed and active stages will not continue downstream.`)) return;
    setBusy(run.id);
    try { await api(`/api/runs/${run.id}/cancel`, { method: 'POST' }); await refresh(); }
